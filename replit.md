...
- **MES — Producción en planta**: full shop-floor tracking module under `/production` (vendedor+).
  - **Tables** (`lib/db/src/schema/production.ts`): `production_locations` (workstations), `production_orders` (number, productId/Name, plannedQty, producedQty, rejectedQty, unit, priority, status pending/in_progress/completed/cancelled, plannedStart/End, startedAt/completedAt, locationId), `production_records` (productionOrderId, locationId, operatorId/Name, qtyProduced, qtyRejected, startTime, endTime, durationMin auto, notes, incident).
  - **Endpoints** under `/api/production/*`: `locations` GET/POST/PATCH; `orders` GET (filter status/locationId)/POST/PATCH (auto-sets startedAt/completedAt on status change)/DELETE (cascades records); `orders/:id/records` GET/POST (validates not-closed, recomputes producedQty/rejectedQty, auto-promotes pending→in_progress and to `completed` when planned reached); `records/:id` DELETE (recomputes aggregates); `dashboard` (counts, totals, KPIs `progressPct`/`unitsPerHour`/`efficiency`, byLocation breakdown).
  - **Pages**: `/production` list (mobile cards on small screens, desktop table; status+location filters, search, create-order and create-location dialogs), `/production/:id` detail (KPI strip + register form with large touch-friendly inputs + history sidebar, polls every 5s, status-transition buttons, validates over-production), `/production/dashboard` (4 status counters + 3 KPI tiles + producción por ubicación table, polls every 10s).
  - Sidebar entry "Producción (MES)" with `Factory` icon. Responsive layout optimized for tablet/celular en planta.
